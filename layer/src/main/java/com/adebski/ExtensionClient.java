...
public class ExtensionClient {
    private static final String EXTENSION_NAME = "hello-world-extension";
    ...
    public static String getNext(final String extensionId) {
        try {
            System.out.println("getNext " + extensionId);
            ...
            if (response.statusCode() == 200) {
                ...
                System.err.printf(
                    "%n Invalid status code %s returned while processing event, response %s",
                    response.statusCode(),
                    response.body());
        } catch (Exception e) {
    ...
    public static String initError(final String extensionId, final String errorType) {
        try {
            System.out.println("Init error " + extensionId + ", " + errorType);
